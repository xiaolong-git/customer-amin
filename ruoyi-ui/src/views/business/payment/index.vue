<template>
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" width="320" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleAddPayRecord(scope.row)"
          >新增回款记录</el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleShowPayRecord(scope.row)"
          >查看回款记录</el-button>
          <el-button
            v-if="scope.row.status !== '5'"
            size="mini"
    ...
    <!-- 添加新增回款记录对话框 -->
    <el-dialog :title="addPayTitle" :visible.sync="addPayDialog" width="600px" append-to-body>
      <el-form ref="addPayForm" :model="addPayForm" :rules="addPayRules" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="回款时间">
              <el-date-picker
                v-model="addPayForm.payTime"
                size="small"
                type="date"
                style="width: 240px"
          ...
          <el-col :span="24">
            <el-form-item label="金额" prop="money">
              <el-input v-model="addPayForm.money" placeholder="金额" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="备注" prop="remark">
              <el-input v-model="addPayForm.remark" placeholder="备注" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitAddPayForm">确 定</el-button>
        <el-button @click="cancelAddPay">取 消</el-button>
      </div>
    </el-dialog>

    <!-- 添加新增回款记录对话框 -->
    <el-dialog :title="showPayTitle" :visible.sync="showPayDialog" width="1000px" append-to-body>
      <el-form ref="showPayForm" label-width="80px">
        <div v-for="(row,index) in showPayRecordList">
          <el-row type="flex" justify="center"  >
            <el-form-item label="回款时间" prop="payTime">
              <el-input v-model="row.payTime" placeholder="回款时间" disabled/>
            </el-form-item>
            <el-form-item label="金额" prop="money">
              <el-input v-model="row.money" placeholder="金额" disabled/>
            </el-form-item>
            <el-form-item label="备注" prop="remark">
              <el-input v-model="row.remark" placeholder="备注" disabled/>
            </el-form-item>
            <el-col :offset="2" :span="4">
              <el-button type="danger" plain size="medium" @click="handleDelPayRecord(row, index)">删除</el-button>
            </el-col>
          </el-row>
        </div>
      </el-form>
...
import { listPaymentRecord, getPaymentRecord, delPaymentRecord, addPaymentRecord, updatePaymentRecord, exportPaymentRecord } from "@/api/business/paymentRecord/paymentRecord";
...
export default {
  ...
  methods: {
    ...
    /** 新增回款记录 */
    handleAddPayRecord(row){
      this.addPayDialog = true
      this.addPayForm = {}
      this.addPayForm.orderId = row.id
    },
    /** 提交 */
    submitAddPayForm(){
      this.addPayDialog = true
      if(!this.addPayForm.orderId){
        this.msgInfo("为获取到订单详情");
        return false
      }
      if(!this.addPayForm.payTime){
        this.msgInfo("请选择时间");
        return false
      }
      if(!this.addPayForm.money){
        this.msgInfo("请填写金额");
        return false
      }

      addPaymentRecord(this.addPayForm).then(response => {
    ...
    /** 查看回款记录 */
    handleShowPayRecord(row){
      this.showPayRecordList = []
      this.showPayDialog = true

      let param = {}
      param.orderId = row.id
      listPaymentRecord(param).then(response => {
        if(response.code === 200){
          this.showPayRecordList = response.rows
        }
      });
    },
    //删除记录
    handleDelPayRecord(row){
      let _this = this;
      this.$confirm('确认该回款记录?', "警告", {
        ...
        type: "warning"
          ...
          let param = []
          param.push(row.id)
          delPaymentRecord(param).then(response => {
          if(response.code === 200){
            _this.showPayRecordList = []
            ...
            listPaymentRecord(param).then(response => {
              if(response.rows){
                _this.showPayRecordList = response.rows
              }
