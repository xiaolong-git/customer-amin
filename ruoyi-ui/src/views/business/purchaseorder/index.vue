      ...
      <el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['business:purchaseorder:query']"
          >查看</el-button>
          <el-button
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPurchaseorderByUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleDetail(row) {
      ...
      getPurchaseorder(id).then(response => {
        ...
        this.open = true;
        this.title = "查看采购订单";
        let productItems = response.data.productItems
        if(productItems){
          this.productList = []
          for (let i=0; i<productItems.length; i++) {
            this.handleChooseBrand(i)
          }
        }
      });
    },
